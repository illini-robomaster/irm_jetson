...
// NOTE: CRC8 works only for total length < 64 bytes.
typedef struct {
  char header[2];
  uint16_t seq_num;
  // data_length might not be necessary because all command length are fixed
  uint8_t data_length; // length of data as in bytes, data length as an arry * 4
                       // = data_length
  uint8_t cmd_id;
  int32_t *data;
...
public:
   ...
   * @note For the smallest length of the packet, see CMD_TO_LEN[] or
   * GetPacketLength()
   */
   ...
   * @note  Flag can only be acquired once. Once asked, flag will be reset to 0
   * (invalid)
   */
  ...
  /**
   * Length of the data section ONLY in bytes. Header/tail/crc8 (total len = 9)
   * NOT included. Gimbal    CMD: id = 0x00, total packet length = 19 - 9 = 10
  ...
  static constexpr uint8_t CMD_TO_LEN[TOTAL_NUM_OF_ID] = {
      sizeof(gimbal_data_t),    sizeof(color_data_t), sizeof(chassis_data_t),
      sizeof(selfcheck_data_t), sizeof(arm_data_t),
  };
  static constexpr uint8_t MAX_PACKET_LENGTH = 33;
  static constexpr uint8_t MIN_PACKET_LENGTH = 10;
  // sum of header and tail = 9. Total packet length = data length (CMD_TO_LEN)
  // + 9
...
private:
   ...
   * @note Only call this function after packet has data and header/tails
   * written The length of CRC8 checksum is 1 byte
   */
